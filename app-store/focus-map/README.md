# 撮影のものさし / Photo Yardstick: App Store公開準備
...
制作背景はアプリ管理側の `app_profiles/focus-map.md`、機能と画面はPhotoYardstickのアプリ側にある現行仕様を参照します。公開文面は利用場面から始め、専門用語と開発工程を初見の入口へ置きません。ディレクトリ名、素材名、過去のcommit記述は履歴と実装の識別子なので一括置換しません。Bundle IDは今回の本人の明示採用に従い更新します。

## 現在の状態

2026-09-05、本人が新名称とBundle IDを採用し、各所への反映を直接依頼した。日英サイト本文・参照原稿・共有画像をローカル更新した。本変更のpush・本番公開・実Forms改名・ASC入力は未実施。[今回の確認記録](site-name-reflection-2026-09-05.md)を参照する。以下の9月4日公開結果と8月31日素材は旧名当時の履歴である。

- App Storeの公開名は、日本語 `撮影のものさし`、英語 `Photo Yardstick` に確定。採用サブタイトルは日本語 `ピントと露出を計算`、英語 `Depth of Field & Exposure`、プロモーション用テキストは両言語とも空欄。認知側の `metadata.*.md` は正本参照と採用値の照合用抜粋だけにする。
...
- 旧公開名と現行slugに対応したプライバシー、セルフサポート、専用Feedback、共通Contactをサイト側で更新し、2026-09-04にGitHub Pagesへ公開済み。新しい日英URLの表示と旧URLの転送なし404を確認した。実フォームの受付有効化は別の未完了ゲート。
...
- 価格は0円、初回配布地域は日本のみ、採用Bundle IDは`com.minatosuzuki.PhotoYardstick`とする。Apple側の登録・保存完了を意味しない。
...
提出用実素材の正本はPhotoYardstick側に置き、Web用の派生素材と区別します。
