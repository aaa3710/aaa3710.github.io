...
**保留（2026-09-04）: 所有者の明示的な再開待ち。** Codexの残作業・実窓口の公開前確認は今実行しない。今回行うのは文書保存だけで、調査、フォーム編集・送信、画面操作、実装、ビルド、push、公開、他タスクへの実行依頼は行わない。

実Googleフォームは未改修・未確認です。既定では埋め込み・外部フォームリンクを表示せず、Feedback／Contactが準備中で送信できないことを日英で明記します。以下のサイト構成・方針の完了は、実際の受付や返信可能を意味しません。

必須同意チェックボックス案は未採用で、専用Feedbackの自由記述1欄を標準として維持します。追加同意の採否とAI処理の実証照合も未完了のため、受付フラグは `false`（未設定）を維持します。受付無効時にはAIの将来利用文を表示しません。
...
- [x] 専用Feedbackを匿名・原則個別返信なし・自由記述1欄とし、アプリ／言語／カテゴリ／端末／OSの再入力をなくす方針を確定
- [ ] 追加同意の採否とAI処理の実証照合を含めて説明・設定を確定し、Feedback／Contactの実Googleフォーム4件を編集して回答者下書き自動保存をすべて無効化。各用途の日英両フォームの質問・説明・必須／任意・情報収集等の設定、異なる入力制限（FeedbackはURL禁止／Contactは必要な公開ページURLを許容）と返信契約を照合後、repository variables `APP_FEEDBACK_READY` / `CONTACT_READY` を文字列 `true` にして有効化
- [ ] 共通公開サポートメールの採否を決め、採用時だけ実値を設定
- [ ] 再開後、4フォームの実設定（メール収集、ログイン要否、公開回答概要、共同編集権限、回答者下書き自動保存等）を確認し、保持・削除、AI処理、同意方式、Privacy／App Privacyの説明と一致させる
- [ ] 日英Supportから該当Feedback／共通Contactへ到達し、最小限の合成入力で送信 → 所有者の受信・通知 → 必要な返信や法定請求対応 → 削除まで、実窓口として機能することを確認する。匿名Feedbackの原則個別返信なしは維持する
...
- [ ] 共通公開サポートメールは任意の審査リスク低減策として採否を決める。メール単独がAppleの一律必須要件とは扱わない
