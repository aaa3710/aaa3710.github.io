...
const content = {
  ja: {
    ...
    testFlight:
      'TestFlightで撮影のものさしをテストしている方は、TestFlightのフィードバック機能をご利用ください。プライバシーの請求は、TestFlightのこのアプリの情報欄にある「Appの詳細」で開発者メールを確認してお送りください。TestFlight由来の情報をこのGoogleフォームへ転送しないでください。',
    consent:
  ...
  en: {
    ...
    testFlight:
      'If you are testing Photo Yardstick with TestFlight, use TestFlight’s feedback feature. For privacy requests, open this app’s page in TestFlight and select App Details in the Information section to find the developer email. Do not forward TestFlight data to this Google Form.',
    consent:
          ...
          <div>
            ...
            {!formReadiness.contact && (
              <p className="intake-status">
                {isEnglish
                  ? 'The contact form is being prepared. Inquiries cannot be submitted from this page yet.'
                  : 'お問い合わせフォームを準備中です。現在、このページからは送信できません。'}
              </p>
            )}
            <p>
        ...
        {formReadiness.contact ? (
          <>
            ...
            >
              ...
              <p>
                <ReadableText>{text.google}</ReadableText>
              </p>
              <p>
                <ReadableText>{text.consent}</ReadableText>
              </p>
          ...
          <section className="form-pending" aria-labelledby="contact-pending">
            <h2 id="contact-pending">
              {isEnglish ? 'About this contact channel' : 'この窓口について'}
            </h2>
            <p>
              {isEnglish
